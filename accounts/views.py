...
def mypage(request):
    ...
    if request.session.pop('show_post_register_message', False):
        messages.success(request, "アカウントが作成されました！<br>まずは、マイページで家族を作成してください。<br>家族を作成することで全機能が使用できるようになります。<br>他の家族メンバーが家族を作成済みであれば、招待URLを発行してもらいましょう。<br>（その場合、家族作成は不要です。）")
...
def portfolio(request):
    return render(request, 'accounts/portfolio.html', {
        ...
        'documents': [
            {'name': '企画書', 'path': '/media/portfolio_docs/0420-FamilyNote-企画書.pdf'},
            {'name': '画面設計図', 'path': '/media/portfolio_docs/0420-FamilyNote-画面設計図.pdf'},
            {'name': '画面遷移図', 'path': '/media/portfolio_docs/0420-FamilyNote-画面遷移図.svg'},
            {'name': 'ER図', 'path': '/media/portfolio_docs/0420-FamilyNote-ER図.svg'},
        ],
        'login_link': '/login/',
    })